...
public class InsuranceSurveyorPolicyHolderDetailView implements Initializable {
    ...
    public InsuranceSurveyorPolicyHolderDetailView(PolicyHolder policyHolder) {
    ...
    private void setValue() {
        pID.textProperty().bind(policyHolder.idProperty());
        ...
        policyOwner.textProperty().bind(policyHolder.policyOwnerIdProperty());
    }
    ...
    public static ObservableList<Claim> getClaim() {
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery(String.format("SELECT * FROM claims WHERE insured_person = '%s'", policyHolder.getId()));
            while (rs.next()) {
    ...
    public static ObservableList<Dependent> getDependent() {
        ...
        try {
            Database.setConnectionDatabase();
            Statement statement = Database.connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            ResultSet rs = statement.executeQuery(String.format("SELECT * FROM dependents WHERE pid = '%s'",  policyHolder.getId()));
            while (rs.next()) {
