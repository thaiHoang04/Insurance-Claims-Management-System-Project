...
import insuranceapp.groupproject_cosc2440_insuranceapp.Controllers.InsuranceManager.PolicyOwnerDetailController;
...
public class InsuranceSurveyorPolicyOwnerDetailView implements Initializable {
    public Label poid;
    public Label name;
    public Label fee;
    ...
    public InsuranceSurveyorPolicyOwnerDetailView(PolicyOwner policyOwner) {
    ...
    public void setValue() {
        poid.textProperty().bind(policyOwner.idProperty());
        name.textProperty().bind(policyOwner.nameProperty());
        fee.textProperty().bind(policyOwner.feeProperty().asString());
    }
    ...
    public static ObservableList<PolicyHolder> getPolicyHolder() {
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery(String.format("SELECT * FROM policy_holders WHERE poid = '%s'", policyOwner.getId()));
            while (rs.next()) {
    ...
    public static ObservableList<Dependent> getDependent() {
        ...
        try {
            Database.setConnectionDatabase();
            Statement statement = Database.connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            ResultSet rs = statement.executeQuery(String.format("SELECT * FROM dependents WHERE poid = '%s'", policyOwner.getId()));
            while (rs.next()) {
                String full_name = rs.getString("full_name");
